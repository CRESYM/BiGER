...
model CombiTimeTable "Table look-up with respect to time and linear/periodic extrapolation methods (data from a two-column table in a .txt file) with one output"
  ...
  Modelica.Blocks.Sources.CombiTimeTable combiTimeTable(extrapolation = Extrapolation, fileName = FileName, smoothness = Smoothness, tableName = TableName, tableOnFile = true) annotation(
