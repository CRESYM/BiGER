...
model ConverterGFL_v4 "Converter model for grid following applications"
...
extends Dynawo.Electrical.Controls.Basics.SwitchOff.SwitchOffGenerator;
 ...
 Modelica.Blocks.Interfaces.RealOutput udPccPu(start = udPcc0Pu) annotation(
    Placement(visible = true, transformation(origin = {69, -12}, extent = {{-5, -5}, {5, 5}}, rotation = 0), iconTransformation(origin = {31, -110}, extent = {{-10, -10}, {10, 10}}, rotation = -90)));
...
//  QGenPu = (uqPccPu*idPccPu - udPccPu*iqPccPu); 
  PGenPu = -1 * ComplexMath.real(uPccPu * ComplexMath.conj(terminal.i));
  QGenPu = -1 * ComplexMath.imag(uPccPu * ComplexMath.conj(terminal.i));

/* controlled voltage source */ 
  idConvPu=idPccPu*ratioTr;
  iqConvPu=iqPccPu*ratioTr;
   
//  UConvPu= sqrt(udConvRefPu*udConvRefPu + uqConvRefPu*uqConvRefPu); //Voltage magnitude
  UConvPu = ComplexMath.'abs'(uPccPu/ratioTr - terminal.i*ratioTr*Complex(R, Xc));
